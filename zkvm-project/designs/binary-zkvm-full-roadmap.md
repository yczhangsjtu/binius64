...
| **M11** | **Verifier succinctness**（用户指定优先，M8-C 后启动）：S1 公开输入 O(T)→O(1)（committed-only 列）+ S2 预处理模型拆分（verifier key / online verify） | 公开输入大小与 T 无关（实测）；online verify 不重建电路；proof 体积亚线性明确不做（Phase 3） | 规划 `designs/verifier-succinctness-plan.md` |
