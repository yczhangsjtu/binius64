...
| **M8-B** | **完整 ISA + 真实工具链**（M8 下半）：mul/div/字节访存/ecall 最小 I/O；riscv32 工具链编译 C/Rust → ELF → tracer → trace；跑真实编译程序 | 编译的 C 程序端到端 prove→verify；与参考模拟器逐指令对拍 | M8-A ✅；任务书 `tasks/M8-B-isa-toolchain.md`（T0 fetch 公开化 / T1 leaf-claim 桥 / T2 ISA / T3 工具链） |
