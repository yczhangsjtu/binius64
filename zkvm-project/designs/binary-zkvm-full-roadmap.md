...
| **M10** | **工程化收官**：库 API 化（prove(program, input)→proof）、CI、文档、安全审查准备 | 外部调用者可 API 驱动；文档完整 | **✅ 已完成（2026-09-08，待验收）**：`vmrs_prove/vmrs_verify`（Proof=transcript bytes，bytes 往返端到端 + 自定义镜像注入）；CI 脚本 `tools/run_zkvm_tests.sh`（实跑 ALL GREEN）；`KNOWN_BOUNDARIES.md`/`SECURITY_REVIEW_PREP.md`/电路构建成本设计（`M10_REPORT.md`） |
