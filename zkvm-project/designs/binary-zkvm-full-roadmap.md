...
| 里程碑 | 内容 | 验收标准 | 依赖 |
|---|---|---|---|
| **M7** | **可扩展 RAM 论证 spike**：路线 A（排序式）与路线 B（Twist 翻译）各一切片，K=2^16、T≥2^12 | 两路线的 gates/prove-time 随 K、T 缩放实测曲线；明确选型 + 报告 | M4 机制 |
| **M8** | **完整 ISA + 真实工具链**：mul/div/字节访存/ecall 最小 I/O；riscv32 工具链编译 C/Rust → ELF → tracer → trace；跑真实编译程序（如排序/哈希/斐波那契） | 编译的 C 程序端到端 prove→verify；与参考模拟器逐指令对拍 | M7（RAM 选型）、M5 |
...
- M7-M10 的任务书在各自启动时生成（同 M2-M6 模式：Leader 写任务书 → Worker 实现 →
  Leader 验收）。
- 当前状态：M6 进行中（固化 + 基准）。**M7 任务书在 M6 验收通过后生成。**
